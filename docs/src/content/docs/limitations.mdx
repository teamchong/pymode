...
| `threading` | Pure Python (`lib/polyfills/threading.py`) | No-op locks for single-threaded WASM. `Thread.start()` raises — use `pymode.parallel` instead. |
...
| `threading` | Polyfilled — no-op locks, `Thread.start()` raises. Use `pymode.parallel` for real parallelism. |
...
**Experimental:** C extension packages compiled to `.wasm` side modules via `scripts/build-extension.py`. Currently supported: `markupsafe`, `simplejson`, `msgpack`.
...
**Heavy packages via child DOs:** Heavy packages (numpy, pandas) have their C extensions compiled to `.wasm` via `zig cc` and loaded via `dl_open`. They can run in child DOs via `pymode.parallel.spawn()`, each with its own 128MB memory and 30s CPU. See [Architecture: Multi-Worker RPC](/pymode/architecture/#multi-worker-rpc-architecture).
