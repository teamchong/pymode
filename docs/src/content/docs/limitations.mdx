...
| **10MB bundle size** | CF Workers' paid-tier limit on compressed deploy. A typical `pymode deploy` produces a 6-8 MB gzipped wasm depending on which packages are AOT-preimported; the remainder is your project sources + site-packages. Heavy native deps (numpy/pandas) consume more of the budget. |
...
|---|---|---|
| numpy | `python-numpy.wasm` variant | ✅ (linalg stubbed) |
| Pillow | `python-pillow.wasm` variant | ✅ |
| pydantic_core | `python-pydantic-core.wasm` variant | ✅ |
...
| markupsafe | statically linked | yes |
| regex / xxhash | statically linked | yes |
| pandas | needs numpy + Cython compile pass | no |
...
| orjson / ujson | Rust/C-only (no pure-Python fallback) | no |
...
For projects needing pandas / scipy / scikit-learn / lxml /
cryptography, **Cloudflare's official Python Workers ships pre-built
wheels for those via Pyodide** — that's the working answer today.
Closing pymode's catalog gap is a multi-month engineering project; we
do it package-by-package as needed.
