...
Wizer pre-initializes the Python runtime at deploy time, reducing cold start from ~28ms to ~5ms. The build script exists (`scripts/build-wizer.sh`) but this feature requires additional testing.
