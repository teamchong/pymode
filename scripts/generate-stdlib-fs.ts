...
const BOOT_FILES = [
    ...
    "_pyio.py",
    "abc.py",
    // archive support (needed by pandas — pd.io.common file detection)
    "tarfile.py",
    "gzip.py",
    // pkgutil (needed by pandas import machinery)
    "pkgutil.py",
    // strptime (pandas tslibs)
    "_strptime.py",
...
function main(): void {
    ...
    // Gzip pre-compression — wrangler will gzip the worker bundle anyway,
    // but pre-compressing the .dat keeps the *raw* asset size down which
    // matters for cold-start parse time. (Brotli would be smaller still,
    // but CF Workers' DecompressionStream doesn't support "br" — only
    // gzip / deflate / deflate-raw.)
    const gzBytes = zlib.gzipSync(jsonBytes, { level: 9 });
    fs.writeFileSync(OUTPUT_DAT, gzBytes);

    const datKb = Math.floor(fs.statSync(OUTPUT_DAT).size / 1024);
    const rawKb = Math.floor(jsonBytes.length / 1024);

    // Write thin TypeScript loader. The .dat file is gzip-compressed JSON.
    // At deploy time the real file gets staged into docs/dist/_pymode/
    // (via the [assets] binding), and the in-bundle copy is stubbed
    // with a tiny "{}" gzip so wrangler doesn't ship 1.2 MB twice.
    // CF Workers disallows async I/O at module init, so decompression
    // happens via DecompressionStream("gzip") on the first call to
    // getStdlibFS(), then the result is cached.
    const loaderContent = `\
...
`;
    ...
    console.log("Done:");
    console.log(`  ${OUTPUT_DAT} (${fileCount} files, ${datKb}KB gzip / ${rawKb}KB raw)`);
